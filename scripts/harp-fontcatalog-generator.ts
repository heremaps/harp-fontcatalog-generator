...
namespace FontCatalogScript {
    ...
    const unicodeBlocks = require("../resources/unicode-ranges.json");
    // tslint:enable
        ...
        await new Promise((resolve, reject) => {
            ...
            if (sdfOptions.charset === "") {
                ...
                generateBMFont(fontPath, sdfOptions, (error: any, textures: any, rawJson: any) => {
                    ...
                    // Save the generated json.
                    fs.writeFileSync(
                        `${assetsDir}/${font.name}/${unicodeBlock.category.replace(
                            / /g,
                            "_"
                        )}.json`,
                        rawJson.data
        ...
        for (const font of fontCatalogDescription.fonts) {
            ...
            const fontObject: Font = {
                ...
                metrics: {
                    ...
                    lineHeight: 0.0,
                    lineGap: Math.round(
                        (fontInfo.lineGap / fontInfo.unitsPerEm) * sdfOptions.fontSize
                    ),
                    capHeight: Math.round(
                        (fontInfo.capHeight / fontInfo.unitsPerEm) * sdfOptions.fontSize
                    ),
                    xHeight: Math.round(
                        (fontInfo.xHeight / fontInfo.unitsPerEm) * sdfOptions.fontSize
                    )
                },
                charset: ""
            };
